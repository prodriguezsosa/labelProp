...
# load data
anes <- readRDS('/Users/pedrorodriguez/Dropbox/GitHub/large_data/conText/data/anes2016.rds')
pre_trained <- readRDS("~/Dropbox/GitHub/large_data/conText/data/glove.rds")  # Glove pre-trained embeddings
transform_matrix <- readRDS("~/Dropbox/GitHub/large_data/conText/data/khodakA.rds")  # transformation matrix (Khodak et al.)

# find vocabulary
feats <- anes2016$response %>% tokens() %>% dfm() %>% dfm_trim(min_docfreq = 5) %>% featfreq() %>% names()

# pre-trained embeddings
glove_subset <- pre_trained[intersect(rownames(pre_trained), feats),]
...
# Seed words to documents

`labelProp` can be combined with `conText` to embed and label short documents. To run these examples you will need to download a larger set of GloVe pre-trained embeddings from...

```{r, message = FALSE}

# tokenize documents
anes_toks <- unique(anes$response) %>% tokens()

# embed documents
anes_dem <- anes_toks %>% 
  dfm %>% 
  dem(pre_trained = pre_trained, transform = TRUE, transform_matrix = transform_matrix, verbose = TRUE)

# seed word vectors
seeds_wvs <- pre_trained[unlist(seeds),]

# node vectors
node_vs <- rbind(as.matrix(anes_dem), seeds_wvs)

# transition matrix
transition_matrix <- build_transition_matrix(x = node_vs, threads = 6L)

# label propagate
texts <- labelProp(x = transition_matrix, seeds = seeds, method = "rw", N = 10, beta = 0.5, as_list = TRUE)
texts <- labelProp(x = rbind(as.matrix(anes_dem), seeds_wvs), seeds = seeds, method = "nns", N = 10, beta = 0.5, as_list = TRUE)

# merge with texts
tokens_subset(anes_toks, docid(anes_toks) %in% texts[["economy"]]$node) %>% sapply(function(x) paste(x, collapse = " "))
tokens_subset(anes_toks, docid(anes_toks) %in% texts[["immigration"]]$node) %>% sapply(function(x) paste(x, collapse = " "))


```

# Seed documents to documents

```{r, message = FALSE}


# seeds
doc_seeds <- list("immigration" = c("text7639", "text2567"), "economy" = c("text2010", "text5925"))

# node vectors
node_vs <- as.matrix(anes_dem)

# transition matrix
transition_matrix <- build_transition_matrix(x = node_vs, threads = 6L)

# label propagate
texts <- labelProp(x = transition_matrix, seeds = doc_seeds, method = "rw", N = 10, beta = 0.5, as_list = TRUE)
texts <- labelProp(x = node_vs, seeds = doc_seeds, method = "nns", N = 10, beta = 0.5, as_list = TRUE)
...
In addition to nearest neighbors we may also be interested in exploring "nearest contexts", namely contexts around a target term that are semantically close to each group's embedding of that term. To do so we use the function `ncs()`. Given ALC embeddings and a set of tokenized contexts (e.g. `immig_toks`), `ncs()` computes the cosine similarities between the ALC group embeddings and the set of tokenized contexts --under the hood, these tokenized contexts are also embedded using ALC. 

```{r}
# compute the cosine similarity between each party's embedding and a set of tokenized contexts
immig_ncs <- ncs(x = immig_wv_party, contexts = immig_toks, N = 5, pre_trained = cr_glove_subset, transform = TRUE, transform_matrix = cr_transform, as_list = TRUE)
...
`conText::get_ncs()` is a wrapper function for `ncs()`, used to compute cosine similarities between group embeddings and single-instance context embeddings with the option to bootstrap standard errors.

```{r, message = FALSE}
# compare nearest neighbors between groups
set.seed(2021L)
immig_party_ncs <- get_ncs(x = immig_toks, N = 10,
                            groups = docvars(immig_toks, 'party'),
                            pre_trained = cr_glove_subset,
                            transform = TRUE,
                            transform_matrix = cr_transform,
